...
class ExecutionManager:
    ...
    def register_task(self, thread_id: str, task: asyncio.Task):
        ...
        app_logger.info(f"ExecutionManager: Registering task {id(task)} for thread {thread_id}")
    ...
    def cancel_task(self, thread_id: str) -> bool:
        ...
        app_logger.info(f"ExecutionManager: Attempting to cancel thread {thread_id}")
        task = self._tasks.get(thread_id)
        if task:
            if not task.done():
                app_logger.info(f"ExecutionManager: Cancelling running task {id(task)} for thread {thread_id}")
                task.cancel()
                return True
            else:
                app_logger.info(f"ExecutionManager: Task {id(task)} for thread {thread_id} is already done.")
        else:
            app_logger.info(f"ExecutionManager: No task found for thread {thread_id}")
        return False
