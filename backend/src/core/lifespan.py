...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        app_logger.warning(f"WARNING: Migration failed (tables may already exist): {e}")
    ...
    try:
        ...
        if client:
            app_logger.info("Langfuse Client Initialized successfully. Sending test trace...")
            ...
            span.end()
            client.flush()
            app_logger.info("Test trace flushed. Check dashboard for 'Startup Test Trace'.")
    except Exception as e:
        app_logger.warning(f"WARNING: Langfuse initialization failed: {e}")
