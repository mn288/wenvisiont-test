...
async def seed_agents():
    ...
    if not os.path.exists(CONFIG_DIR):
        app_logger.warning(f"SEEDER: Config directory not found at {CONFIG_DIR}")
        ...
        if os.path.exists(local_path):
            ...
            app_logger.info("SEEDER: No agent config found. Skipping seed.")
            return
    else:
    ...
    if not files:
        app_logger.info("SEEDER: No YAML files found in config/agents.")
        return

    app_logger.info(f"SEEDER: Found {len(files)} agent configs. seeding...")
    ...
    for file_path in files:
        try:
            ...
            try:
                ...
                app_logger.info(f"SEEDER: Seeded agent '{node_config.name}'")
            except Exception as e:
                app_logger.error(f"SEEDER: Failed to parse {file_path}: {e}")

        except Exception as e:
            app_logger.error(f"SEEDER: Error reading {file_path}: {e}")

    app_logger.info("SEEDER: Agent seeding complete.")
...
async def seed_mcp_servers():
    ...
    try:
        ...
        if not existing:
            app_logger.info("SEEDER: Creating default 'filesystem' MCP server (SSE)...")
            ...
            app_logger.info("SEEDER: 'filesystem' MCP server created successfully.")
    except Exception as e:
        app_logger.error(f"SEEDER: Failed to seed filesystem MCP: {e}")

    # 2. S3 SERVER
    try:
        existing_s3 = await mcp_service.get_servers_by_names(["s3"])
        if not existing_s3:
            app_logger.info("SEEDER: Creating default 's3' MCP server (SSE)...")
            ...
            app_logger.info("SEEDER: 's3' MCP server created successfully.")
    except Exception as e:
        app_logger.error(f"SEEDER: Failed to seed S3 MCP: {e}")

    # 3. MATH SERVER
    try:
        existing_math = await mcp_service.get_servers_by_names(["mcp-math"])
        if not existing_math:
            app_logger.info("SEEDER: Creating default 'mcp-math' MCP server (SSE)...")
            ...
            app_logger.info("SEEDER: 'mcp-math' MCP server created successfully.")
    except Exception as e:
        app_logger.error(f"SEEDER: Failed to seed Math MCP: {e}")

    # 4. ANALYSIS SERVER
    try:
        existing_analysis = await mcp_service.get_servers_by_names(["analysis-mcp"])
        if not existing_analysis:
            app_logger.info("SEEDER: Creating default 'analysis-mcp' MCP server (SSE)...")
            ...
            app_logger.info("SEEDER: 'analysis-mcp' MCP server created successfully.")
    except Exception as e:
        app_logger.error(f"SEEDER: Failed to seed Analysis MCP: {e}")
    # 5. GCP RAG SERVER
    try:
        existing_rag = await mcp_service.get_servers_by_names(["gcp-rag"])
        if not existing_rag:
            app_logger.info("SEEDER: Creating default 'gcp-rag' MCP server (STDIO)...")
            ...
            app_logger.info("SEEDER: 'gcp-rag' MCP server created successfully.")
    except Exception as e:
        app_logger.error(f"SEEDER: Failed to seed GCP RAG MCP: {e}")

    # 6. GCP BIGQUERY SERVER (STDIO via Python)
    try:
        existing_bq = await mcp_service.get_servers_by_names(["gcp-bigquery"])
        if not existing_bq:
            app_logger.info("SEEDER: Creating default 'gcp-bigquery' MCP server (STDIO)...")
            ...
            app_logger.info("SEEDER: 'gcp-bigquery' MCP server created successfully.")
    except Exception as e:
        app_logger.error(f"SEEDER: Failed to seed GCP BigQuery MCP: {e}")
